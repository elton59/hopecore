...
if(isset($_POST['createdrug']))
{
    ...
    $result=$mysqli->query("UPDATE lab_test set prescription='$prpres',drugname='$apdatid',drug_cost='$prcost',status='prescription_completed' where id='$prid'")or die($mysqli->error);
...
if(isset($_POST['pharmacist_login']))
{
   ...
   $result=$mysqli->query("SELECT * from users where email='$phemail' and password='$phpass' and specialization ='pharmacist' and status='approved' limit 1")or die($mysqli->error);
   $row=$result->fetch_assoc();
   $_SESSION['pharmacist']=$phemail;
	$count=mysqli_num_rows($result);
		  if($count==1)
	{
