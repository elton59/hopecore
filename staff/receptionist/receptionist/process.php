...
if(isset($_POST['createappointment']))
{
    ...
    $result=$mysqli->query("INSERT INTO appointments(specialization,appointmenttype,patient_email,staffid) values('$apspec','$aptype','$appatid','$apstid')")or die($mysqli->error);
...
if(isset($_POST['editappointment']))
{
    $id=$_POST['id'];
    $apstid=$_POST['apstid'];
    $apstide=$_POST['apstide'];
  
   $result=$mysqli->query("UPDATE  appointments SET comment='$apstid', date_of_appointment='$apstide' where id='$id'") or die($mysqli->error);
...
if(isset($_POST['receptionist_login']))
{
   ...
   $result=$mysqli->query("SELECT * froM users where email='$remail' AND password='$rpass' and specialization='receptionist' limit 1")or die($mysqli->error);
   $row=$result->fetch_assoc();
...
if(isset($_POST['finance_login']))
{
   ...
   $result=$mysqli->query("SELECT * from users where email='$fnemail' and password='$fnpass' and specialization ='finance' limit 1")or die($mysqli->error);
   $row=$result->fetch_assoc();
   $_SESSION['finance']=$fnemail;
		  ...
		  if($count==1)
	{
		...
		echo "<script>alert('login success')
			  location.replace('./management/finance/main.php');
			   </script>
			   ";
	}
	elseif($count!==1)
