...
if(isset($_POST['record-test-doctor']))
{
    $id=$_POST['id'];
    $vdate=$_POST['vdate'];
    $vreport=$_POST['vreport'];
    
   


    $result=$mysqli->query("UPDATE lab_test SET nchv_visit_date='$vdate',nchv_comment='$vreport' where id='$id'") or die($mysqli->error);

    if($result) 
    {
        echo
        "<script>alert('operation successfull');
        window.location.replace('newvisit.php')
        </script>
        ";
    }
    else
...
{
    $vreport=$_POST['vreport'];
    $vpatname=$_POST['vpatname'];
    $vdate=$_POST['vdate'];

    $result=$mysqli->query("INSERT INTO healthreport(report,patient_email,visit_date) values('$vreport','$vpatname','$vdate')") or die($mysqli->error);

    if($result)
    {
        echo
        "<script>alert('operation successfull');
        window.location.replace('viewvisit.php')
        </script>
        ";
    }
    else
...
//nchv Login
if(isset($_POST['nchv_login']))
{
    session_start();
    $ncemail=$_POST['ncemail'];
    $ncpass=$_POST['ncpass'];

   $result=$mysqli->query("SELECT * from users where email='$ncemail' and password='$ncpass' and specialization ='nchv'  and status='approved' limit 1")or die($mysqli->error);
   $row=$result->fetch_assoc();
   $_SESSION['nchv']=$ncemail;
	$count=mysqli_num_rows($result);
		  if($count==1)
	{
