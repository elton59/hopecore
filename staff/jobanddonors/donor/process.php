...
if(isset($_POST['createdonor']))
{
    ...
    $result=$mysqli->query("INSERT INTO users(email,password,firstname,lastname,gender) values('$donemail','$donpass','$donfname','$donlname','$dongen')") or die($mysqli->error);
...
if(isset($_POST['createfeedback']))
{
    ...
    $result=$mysqli->query("INSERT INTO feedback(firstname,lastname,phonenumber,message,receiver,reply,email) values('$fdfname','$fdlname','$fdpno','$fdmessage','$fdreceiver','$fdreply','$fdemail')")or die($mysqli->error);

    if($result)
    {
        echo
        "<script>alert('operation successfull');
        window.location.replace('admin/viewfeedback.php')
        </script>
        ";
    }
    else
...
if(isset($_POST['createtreatment']))
{
    ...
    $result=$mysqli->query("INSERT INTO treatment(treatment,staffid,patientid,drugid) values('$trtr','$trsid','$trid','$trdid')")or die($mysqli->error);
...
if(isset($_POST['createdonation']))
{
    ...
    $result=$mysqli->query("INSERT INTO donation(fullname,donationtype,amount,transactionid,userid) values('$donfname','$dontype','$donamnt','$dontid','$donuid')")or die($mysqli->error);

    if($result)
    {
        echo
        "<script>alert('operation successfull');
        window.location.replace('viewdonation.php')
        </script>
        ";
    }
    else
...
//donors Login
if(isset($_POST['donors_login']))
{
    session_start();
    $donemail=$_POST['donemail'];
    $donpass=$_POST['donpass'];

   $result=$mysqli->query("SELECT * from users where email='$donemail' and password='$donpass' and specialization='donor' and status='approved' limit 1")or die($mysqli->error);
   $row=$result->fetch_assoc();
   $_SESSION['donor']=$donemail;
	$count=mysqli_num_rows($result);
		  if($count==1)
	{
