...
if(isset($_POST['driver_login']))
{
   ...
   $result=$mysqli->query("SELECT * from users where email='$dremail' and password='$drpass' and specialization ='driver' and status='approved' limit 1")or die($mysqli->error);
   $row=$result->fetch_assoc();
   $_SESSION['driver']=$dremail;
	$count=mysqli_num_rows($result);
		  if($count==1)
	{
