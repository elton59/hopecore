...
//Create Donors
if(isset($_POST['record-test-results']))
{
    $id=$_POST['id'];
    $lab_test_type=$_POST['lab_test_type'];
    $lab_test_date=$_POST['lab_test_date'];
    $lab_test_results=$_POST['lab_test_results'];
    $lab_test_cost=$_POST['lab_test_cost'];
    $lab_test_comment=$_POST['lab_test_comment'];
    $lab_test_quantity=$_POST['lab_test_quantity'];
    $stock_in_out=$_POST['stock_in_out'];
    $fstock=$stock_in_out+$lab_test_quantity;
    $lab_test_cost2= $lab_test_quantity * $lab_test_cost;
    
    $result2=$mysqli->query("update medicalsupplies set stock_in_out='$fstock' where goodname='$lab_test_type'") or die($mysqli->error);
    $result=$mysqli->query("update lab_test set date_of_test=' $lab_test_date',results=' $lab_test_results',lab_test_cost='$lab_test_cost2',labtech_comment='  $lab_test_comment',prescription='$lab_test_quantity',status='test_completed' where id='$id'") or die($mysqli->error);
...
if(isset($_POST['lab_login']))
{
   ...
   $result=$mysqli->query("SELECT * from users where email='$labemail' and password='$labpass' and specialization ='lab' and status='approved' limit 1")or die($mysqli->error);
   $row=$result->fetch_assoc();
   $_SESSION['lab']=$labemail;
	$count=mysqli_num_rows($result);
		  if($count==1)
	{
