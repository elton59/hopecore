...
if(isset($_POST['createstaff']))
{
    ...
    $result2=$mysqli->query("INSERT INTO jobaplicant(specialization,profession,firstname,lastname,gender,adress,serviceid,phonenumber,email,password) values('$staffspec','$staffprof','$stafffname','$stafflname','$staffgen','$staffadr','$staffserid','$staffpno','$staffemail','$staffpass')") or die($mysqli->error);
...
{
    session_start();
    $jbemail=$_POST['jbemail'];
    $jbpass=$_POST['jbpass'];

   $result=$mysqli->query("SELECT * from users where email='$jbemail' and password='$jbpass' and specialization ='applicant' and status='approved' limit 1")or die($mysqli->error);
   $row=$result->fetch_assoc();
   $_SESSION['applicant']=$jbemail;
	$count=mysqli_num_rows($result);
		  if($count==1)
	{
