...
if(isset($_POST['createsupply']))
{
    ...
    $result=$mysqli->query("INSERT INTO medicalsupplies(company,userid,deliverydate,expirydate,weight,driver_email,stockquantity,stock_in_out,goodname,category) values('$supcomp','$supuid','$supdelivery','$supexdate','$supweight','$supdemail','$supsin','$supsqty','$supgname','$cat')")or die($mysqli->error);
...
if(isset($_POST['editsupply']))
{
   ...
   $result=$mysqli->query("UPDATE  medicalsupplies SET stockquantity='$supqty'where id='$supid'") or die($mysqli->error);
...
//finance Login
if(isset($_POST['shipmentmanager_login']))
{
    session_start();
    $shemail=$_POST['shemail'];
    $shpass=$_POST['shpass'];

   $result=$mysqli->query("SELECT * from users where email='$shemail' and password='$shpass' and specialization ='shipment_manager  ' and status='approved' limit 1")or die($mysqli->error);
   $row=$result->fetch_assoc();
   $_SESSION['shipment']=$shemail;
	$count=mysqli_num_rows($result);
		  if($count==1)
	{
