...
if(isset($_POST['createdrug']))
{
    ...
    $result=$mysqli->query("INSERT INTO drugs(prescription,fullname) values('$drpres','$drfuname')")or die($mysqli->error);
...
{
    $id=$_POST['id'];
    $drpres=$_POST['prescription'];
    $drfuname=$_POST['drfuname'];

  
   $result=$mysqli->query("UPDATE  drugs SET prescription='$drpres',fullname='$drfuname'where id='$id'") or die($mysqli->error);
...
if(isset($_POST['nurse_login']))
{
   ...
   $result=$mysqli->query("SELECT * from users where email='$nemail' and password='$npass' and specialization ='nurse' and status='approved' limit 1")or die($mysqli->error);
   $row=$result->fetch_assoc();
   $_SESSION['nurse']=$nemail;
	$count=mysqli_num_rows($result);
		  if($count==1)
	{
