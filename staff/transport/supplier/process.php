...
{
    $supcomp=$_POST['supcomp'];
    $supuid=$_POST['supuid'];
    $supgname=$_POST['supgname'];
    $supdelivery=$_POST['supdelivery'];
    $supexdate=$_POST['supexdate'];
    $supweight=$_POST['supweight'];
    $supsqty=$_POST['supsqty'];
    $supdemail=$_POST['supdemail'];
    $supsin=$_POST['supsin'];
  

    $result=$mysqli->query("INSERT INTO medicalsupplies(company,userid,deliverydate,expirydate,weight,driver_email,stock_in_out,stockquantity,goodname) values('$supcomp','$supuid','$supdelivery','$supexdate','$supweight','$supdemail','$supsin','$supsqty','$supgname')")or die($mysqli->error);
...
if(isset($_POST['editsupply']))
{
    ...
    $supamnt=$_POST['supamnt'];
   $result=$mysqli->query("UPDATE  medicalsupplies SET comment='$supcom',cost='$supamnt',status='$supstatus' where id='$id'") or die($mysqli->error);
...
//supplier Login
if(isset($_POST['suppliers_login']))
{
    session_start();
    $semail=$_POST['semail'];
    $spass=$_POST['spass'];

   $result=$mysqli->query("SELECT * from users where email='$semail' and password='$spass' and specialization ='supplier' and status='approved' limit 1")or die($mysqli->error);
   $row=$result->fetch_assoc();
   $_SESSION['supplier']=$semail;
	$count=mysqli_num_rows($result);
		  if($count==1)
	{
